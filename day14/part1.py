...
def compute(s: str) -> int:
    ...
    OFFSET = min(p.x for rock_path in rock_paths for p in rock_path)
    WIDTH = max(p.x for rock_path in rock_paths for p in rock_path) - OFFSET + 1
    HEIGHT = max(p.y for rock_path in rock_paths for p in rock_path) + 1
...
def pour_one(cave, entry):
    p = entry
    while p.y < HEIGHT - 1 and 0 < p.x < WIDTH - 1:
